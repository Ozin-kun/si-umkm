...
<x-app-layout>
                                    ...
                                    <tr class="border-b border-slate-200 hover:bg-slate-50">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center justify-center gap-2">                                                
                                                ...
                                                <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" class="m-0 flex">
                                                    @csrf
                                                    @method('DELETE')                                                    
                                                    <button type="button" class="inline-flex w-20 justify-center items-center rounded-full bg-rose-50 px-3 py-1.5 text-xs font-medium text-rose-700 transition-colors hover:bg-rose-100" @click="confirmDeleteCategory($el.closest('form'), @js($category->name))">
                                                        Hapus
                                                    </button>
                                                </form>
                                            </div>
                ...
                <form :action="formAction" method="POST" class="px-6 py-6 sm:px-8">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input x-ref="nameInput" type="text" name="name" x-model="form.name" placeholder="Masukkan nama kategori..." required maxlength="100" class="mt-1 block w-full rounded-2xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-600">Deskripsi</label>
                            <textarea name="description" rows="4" x-model="form.description" placeholder="Deskripsikan kategori ini..." class="mt-1 block w-full rounded-2xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
                        </div>
                    </div>
    ...
    >
        ...
        <div class="relative z-10 w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
            <div class="flex items-start justify-between border-b border-slate-100 px-6 py-4 sm:px-8">
                <div>
                    <h3 id="create-category-title" class="text-lg font-semibold tracking-tight text-slate-900">Tambah Kategori</h3>
                    <p class="mt-1 text-sm text-slate-500">Tambahkan kategori baru untuk digunakan UMKM.</p>
                </div>
            ...
            <form action="{{ route('admin.categories.store') }}" method="POST" class="px-6 py-6 sm:px-8">
                @csrf

                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-600">Nama Kategori</label>
                        <input type="text" name="name" placeholder="Contoh: Kuliner, Fashion, Kerajinan..." required maxlength="100" class="mt-1 block w-full rounded-2xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-600">Deskripsi</label>
                        <textarea name="description" placeholder="Deskripsikan kategori ini..." rows="4" class="mt-1 block w-full rounded-2xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
                    </div>
                </div>
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="rounded-full bg-indigo-600 px-4 py-2.5 font-medium text-white transition-colors hover:bg-indigo-700">Simpan Kategori</button>
                </div>
            </form>
        </div>
    </div>
